...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        print(args)
        ret = scheme_eval(procedure.body, frame)
        return ret
...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Error in funciton 'apply_primitive': " + err)
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("the number of FORMALS doesn't match the number of values input to 'make_call_frame'")
...
def do_lambda_form(vals, env):
    ...
    if len(vals) > 2:
        begin_form = Pair("begin", vals.second)
        return LambdaProcedure(formals, begin_form, env)
...
def do_mu_form(vals):
    ...
    if len(vals) > 2:
        begin_form = Pair("begin", vals.second)
        return MuProcedure(formals, begin_form)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_form = do_lambda_form(Pair(target.second, vals.second), env)
        check_formals([target.first])
...
def do_let_form(vals, env):
    ...
    for val in bindings:
        ...
        values = Pair(val.second.first, values)
...
def do_or_form(vals, env):
    ...
    for val in vals:
        if scheme_eval(val, env) is not False:
            return val
    return False
    ...
    for i, clause in enumerate(vals):
        ...
        if clause.first == "else":
            ...
            if clause.second is nil:
                raise SchemeError("badly formed else clause. The proper format is (else <expr>)")
...
def check_formals(formals):
    ...
    for formal in formals:
        if scheme_symbolp(formal) and formal not in set:
            set[formal] = True
        else:
            raise SchemeError("FORMAL value is not valid. Check your function input and make sure it's a proper symbol and isn't repeated")
